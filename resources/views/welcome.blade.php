...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .dots span:first-child { background: #111; }
        ...
        @media (max-width: 760px) {
            ...
            .top-header { grid-template-columns: 1fr; justify-items: start; padding-top: 10px; padding-bottom: 10px; }
            .top-header {
                gap: 12px;
            }
            .header-nav {
            ...
            .hero-copy { margin-left: 18px; padding: 72px 0 42px; }
            .categories,
            .product-grid,
            .movement-grid,
            .feature-banner,
...
<body>
            ...
            <section id="new-arrivals" class="section">
                <div class="section-head">
                    ...
                    <div class="section-arrows">
                        <span id="latest-prev" aria-label="View previous shoes">‹</span>
                        <span id="latest-next" aria-label="View more shoes">›</span>
                    </div>
                </div>

                <div class="product-carousel">
                    <div class="product-page is-active" data-page="0">
                ...
                <div class="movement-grid">
                    <article class="movement-card">
                        ...
                        <div class="movement-meta">
                            ...
                            <span class="look-link">Follow us</span>
                        </div>
                    </article>
                    <article class="movement-card">
    ...
    <script>
        const latestPages = Array.from(document.querySelectorAll('.product-page'));
        const latestDots = Array.from(document.querySelectorAll('[data-dot]'));
        const prevButton = document.getElementById('latest-prev');
        const nextButton = document.getElementById('latest-next');
        let currentPage = 0;

        function renderLatestPage(index) {
            const nextPageIndex = (index + latestPages.length) % latestPages.length;

            latestPages.forEach((page, pageIndex) => {
                page.classList.toggle('is-active', pageIndex === nextPageIndex);
            });

            latestDots.forEach((dot, dotIndex) => {
                dot.style.background = dotIndex === nextPageIndex ? '#111' : '#d4d1cb';
            });

            currentPage = nextPageIndex;
        }

        prevButton.addEventListener('click', () => renderLatestPage(currentPage - 1));
        nextButton.addEventListener('click', () => renderLatestPage(currentPage + 1));

        latestDots.forEach((dot, dotIndex) => {
            dot.addEventListener('click', () => renderLatestPage(dotIndex));
        });
